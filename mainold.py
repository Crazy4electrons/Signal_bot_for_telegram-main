...
# Lock to ensure only one trade signal is processed at a time (prevents race conditions)
trade_sequence_lock: asyncio.Lock = asyncio.Lock()
...
SIGNAL_TIMEZONE =pytz.timezone('Etc/GMT+4')#  gmt-4
LOCAL_TIMEZONE =pytz.timezone('Etc/GMT-2')# gmt+2
...
def release_processing_lock() -> None:
    """
    Safely release the global trade_sequence_lock (if locked) and set
    is_processing_trade_sequence to False. Centralizes logging and
    exception handling so callers don't repeat the same code.
    """
    global is_processing_trade_sequence
    try:
        if trade_sequence_lock.locked():
            trade_sequence_lock.release()
            logger.debug("Released trade_sequence_lock.")
    except Exception:
        logger.exception("Error releasing trade_sequence_lock")
    finally:
        # Ensure the global processing flag is always reset
        is_processing_trade_sequence = False


def reset_trade_sequence_state() -> None:
    """
    Reset the trade_sequence_state to its default values used at the
    start of a new sequence. Call this in error paths or when finishing
    a sequence to avoid repeating the update logic.
    """
...
async def trade_signal_webhook(request: Request) -> JSONResponse:
    ...
    # Try to acquire the lock quickly; if it's already held, respond that the signal is ignored.
    try:
        await asyncio.wait_for(trade_sequence_lock.acquire(), timeout=0.01)
        # Mark processing started immediately after acquiring the lock to avoid race conditions
        is_processing_trade_sequence = True
        logger.debug("Acquired trade_sequence_lock for incoming signal; proceeding to process it.")
    ...
    if not pocket_option_client or not pocket_option_client.is_connected:
        ...
        if await connect_pocket_option_client():
            ...
            logger.critical("Failed to connect Pocket Option client upon receiving trade signal.")
            # Centralized cleanup
    ...
    if raw_notification_text.strip().endswith("Youseff") or raw_notification_text.strip().endswith("Magic"):
        SIGNAL_TIMEZONE =pytz.timezone('Etc/GMT+5')#  gmt-5
        logger.info("Using GMT-5 timezone for signal parsing based on notification content.")
    else:
        SIGNAL_TIMEZONE =pytz.timezone('Etc/GMT+4')#  gmt-4
        logger.info("Using GMT-4 timezone for signal parsing based on notification content.")
    ...
    latency_sum = float(0)
    for _ in range(10):
        logger.debug("== api-eu.po.market ==")
        res = measure_one("demo-api-eu.po.market")
        dns = res.get("dns_ms")
        logger.debug(f"  DNS: {dns:.1f} ms")
    ...
    # NOTE: lock already acquired and is_processing_trade_sequence set above.
    ...
    try:
        while not pocket_option_client.is_connected or not pocket_option_client:# type: ignore
            logger.warning("Pocket Option client disconnected before placing initial trade. Attempting to reconnect...")
            if await connect_pocket_option_client():
                logger.info("Reconnected Pocket Option client successfully.")
            else:
                logger.error("Reconnection attempt failed. Retrying in 5 seconds...")
                if datetime.now(LOCAL_TIMEZONE) > target_local_dt + timedelta(seconds=2):
                    # Ensure the trade sequence state is reset before releasing the global processing lock
                    reset_trade_sequence_state()
                    release_processing_lock()
                    logger.error("Exceeded allowable time to place initial trade after reconnection attempts. Aborting trade.")
                    raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Pocket Option API not connected and reconnection failed before initial trade.")
...
async def handle_trade_outcome_and_martingale(trade_id: int|str, duration: int, asset: str, direction: OrderDirection, amount: float,after_entry_balance:float,entry_time:datetime) -> None:
    ...
    trade_end_time = (((entry_time+timedelta(seconds=duration))-(datetime.now(LOCAL_TIMEZONE))) + timedelta(seconds=0.01)).total_seconds()
    ...
    try:
        ...
        if current_balance.balance > after_entry_balance:
            ...
            # Centralized cleanup: reset state and release the processing lock so new signals can be processed
            reset_trade_sequence_state()
            release_processing_lock()
            martingale_reentry_needed = False
        else:
        ...
        martingale_reentry_needed = False
        # Centralized cleanup: ensure state reset in case of errors so we don't block future trades
        reset_trade_sequence_state()
        release_processing_lock()
    ...
    logger.info(f"Checking official final outcome: Trade placed at {trade_sequence_state['last_trade_open_time']}; amount: {trade_sequence_state['current_amount']}; asset: {asset}; direction: {direction.value}")
    if pocket_option_client and pocket_option_client.is_connected:
        try:
            if trade_sequence_state["last_trade_status"] == "win":
                profit = (await pocket_option_client.get_balance()).balance - after_entry_balance - trade_sequence_state["current_amount"] # type: ignore
                logger.info(f"\n\nOFFICIAL FINAL OUTCOME for Trade ID {trade_id}\n\nStatus:{trade_sequence_state['last_trade_status'].upper()}\nProfit: {profit:2f}) USD.\n\n")
            else:
                logger.info(f"OFFICIAL FINAL OUTCOME for Trade ID {trade_id}:\n {trade_sequence_state['last_trade_status'].upper()}.")
    ...
    logger.info(f"\n\nMartingale Sequence State AFTER processing Trade ID {trade_id}:\nActive={trade_sequence_state['active']}\nLevel={trade_sequence_state['current_level']}\nAmount={trade_sequence_state['current_amount']:.2f}\nGlobal Processing Lock: {is_processing_trade_sequence}") 
